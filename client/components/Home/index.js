...
import {
  Button, Row, Col, Typography, Alert,
} from 'antd';
import { HomeTwoTone } from '@ant-design/icons';
import { useQueryParam, StringParam } from 'use-query-params';

import { setAllTokens, ServerGameStatus, images } from 'Utilities/common';
import { getGame, getNewGame, deleteGameOverNote } from 'Utilities/services/gameService';
import {
  getActiveGames, getUserData, getNotifications, acceptGameRequest, rejectGameRequest,
  acceptFriendRequest, rejectFriendRequest, addFriend,
              ...
              {notifications && notifications.length > 0
                ? notifications.map((n) => (
                  <NotificationCard
                    key={n.id}
                    notification={n}
                    handleAcceptGame={handleAcceptGame}
                    handleRejectGame={handleRejectGame}
                    handleAcceptFriend={handleAcceptFriend}
                    handleRejectFriend={handleRejectFriend}
                    handleSeeProfile={handleSeeProfile}
                    handleSeeGame={handleSeeGame}
                  />
                ))

...
const { generateWords, createGameRequest } = require('@util/gameUtil');
...
const {
  NUM_ROUNDS, ROUND_LEN, NUM_HIGH_SCORES, ServerGameStatus, ServerRoundState,
} = common;
...
const getActive = async (req, res) => {
  ...
  /* if a game is pending and awaiting the requester's acceptance,
  it is NOT considered an active game */
  const filteredAg = activeGames.filter((ag) => !(ag.status === ServerGameStatus.pending
    && ag.player2._id.toString() === userId.toString()));
...
const sendRound = async (req, res) => {
  ...
  if (type === 'doodle') {
    ...
    for (let i = 0; i < gameData.doodles.length; i += 1) {
      const doodle = gameData.doodles[i];
    ...
    game.activePlayer = game.activePlayer.toString() === game.player1.toString()
      ? game.player2 : game.player1;

    // send email update to new active player
    const inactive = game.activePlayer.toString() === game.player1.toString()
      ? await User.findById(game.player2) : await User.findById(game.player1);
    const active = await User.findById(game.activePlayer);
    if (active.settings.alertFrequency === 'ALL') {
      sendUpdate(active.email, `It's your turn against <b style="color:tomato;">${inactive.username}</b>`,
    ...
    gameData.guesses.forEach((guess) => {
      ...
      if (guess.isCorrect) {
        roundScore.roundTotals.numCorrect += 1;
        game.result.gameTotals.numCorrect += 1;
      }
  ...
  if (currentRound.state === ServerRoundState.over) {
    if (game.rounds.length < game.numRounds) {
      ...
      const gon = new GameOverNote({
        ...
        receiver: game.player1.toString() === game.activePlayer.toString()
          ? game.player2 : game.player1,
        isActive: true,
      });
      await gon.save();
      ...
      if (inactive.settings.alertFrequency === 'ALL') {
        sendUpdate(inactive.email, `Game with <b style="color:tomato;">${active.username}</b> 
      ...
      /* if player has less than NUM_HIGH_SCORES total scores, just add this game to the high score
      list */
      if (p1.highScores.length < NUM_HIGH_SCORES) {
      ...
      } else if (p1.highScores
        .sort((a, b) => a.score.totalTimeSpent - b.score.totalTimeSpent)[NUM_HIGH_SCORES - 1]
        > game.result.gameTotals.totalTimeSpent) {
        ...
        p1.highScores
          .sort((a, b) => a.score.totalTimeSpent - b.score.totalTimeSpent); // sort ascending
        game.isHighScore.p1 = true;
      }

      /* if player has less than NUM_HIGH_SCORES total scores, just add this game to the high
      score list */
  ...
  if (savedGame.status === ServerGameStatus.pending) {
    const gameRequestId = await createGameRequest(req,
      savedGame.player1._id, savedGame.player2._id, savedGame._id);
    // if player2 has email notifications on, send email
    if (savedGame.player2.settings.alertFrequency === 'ALL') {
      sendUpdate(savedGame.player2.email, `You have a new game request from 
  ...
  return res.status(201).json(savedGame.toJSON());
};
...
module.exports = {
  getActive, sendRound, getNewGame, getGame, getRandomDoodle, deleteNote, getRoundReplay,
};
